...
The tSNE provided in the object that was imported is saved into the variable `dim_red` and all the dimensional reductions from the `sce` object removed. In addition, it is determined the `logcounts` from the `raw` counts (available at `assay(altExp(sce, "raw"), "X")`) applying the formula: `log1p(x/sum(x)*10000)` (running `log.counts <- apply(counts, 2, function(x) log1p(x/sum(x)*10000))`). 

```{r tsne & rm dimreds & logcounts, echo=TRUE, message=FALSE, warning=FALSE}
## Save tSNE
