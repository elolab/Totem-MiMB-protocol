...
In order to run `Totem`, one needs only log normalized single-cell gene expression data. `Totem` is a novel trajectory inference method for tree-shaped cell trajectory topologies [@smolander2022totem]. It relies mainly on three steps: (1) clustering; (2) Minimum Spanning Trees (MSTs); and (3) smoothing MSTs with the principal curves algorithm. In between these steps, there are a serial of intermediate steps to handle the data and tune some parameters based on visualizations and user knowledge. `Totem` introduces the concept of **cell connectivity** which consists on the average of connections that a cell has through a set of clustering results which helps the user to identify and support the potential starting and ending points of a trajectory. In this tutorial We will go through the main steps which are listed below.   
...
The applicability of Totem for cell trajectory inference (TI) is demonstrated with a data set that met the following criteria:
   ...
   4. real world data (non-simulated). 
...
dimred2cluster <- "lmds"
if (dimred2cluster=="lmds") {
  sce <- RunDimRed(object = sce,
                   dim.red.method = "lmds",
                   dim.red.features = hvg,
                   dim.reduction.par.list = list(ndim=5))
}
if (dimred2cluster=="pca") {
  sce <- RunDimRed(object = sce,
                   dim.red.method = "pca",
                   dim.red.features = hvg,
                   dim.reduction.par.list = list(ndim=10))
}
```
...
**Cell connectivity** is a novel concept introduced in `Totem`. For a given clustering result and the respective MST (Minimum Spanning Trees) , the connectivity of a cell cluster consists in the ratio of its connections to other clusters by the total number of clusters. The cell connectivity is scaled by the maximum value and averaged across a set of clustering and MST results (by default 1K). Higher the value farther the distance for the leafs/ending points of the trajectory and, thus, more likely to represent, e.g., bifurcations points (see Fig.4 [@smolander2022totem]).
...
At this stage the root can be defined be giving the cluster number. Let's define cluster 4 as root. 
...
Since we are using a data set for which is known is truth pseudotime and respective trajectory, We compare below this "ground-truth" with the result predicted by `Totem` (on the left side). 
